...
const vehicleUrl = c => `cars/${slugify(`${c.year}-${c.brand}-${c.model}`)}`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (feat) {
    const homePriority = ['c43','c45','c46','c47','c48','c44'];
      ...
      btn.addEventListener('click', () => {
        ...
        render('inventory-grid', pool.length
          ...
          : `<div style="grid-column:1/-1;text-align:center;padding:80px 20px;color:var(--muted)">
               ...
               <p>Try a different filter or <a href="order" style="color:var(--red3)">place a custom order.</a></p>
             </div>`
        );
      });
    });
  ...
  if (brandsEl) {
    render('brands-grid', D.brands.map(b => {
      ...
      return `
        <a class="brand-card rv" data-origin="${b.origin}" href="inventory?q=${encodeURIComponent(b.name)}" aria-label="Browse ${b.name} cars for sale in Uganda">
...
function doSearch() {
  ...
  render(target.id, res.length
    ...
    : `<div style="grid-column:1/-1;text-align:center;padding:80px 20px;color:var(--muted)">
         ...
         <p>Try adjusting your search or <a href="order" style="color:var(--red3)">place a custom order.</a></p>
       </div>`
  );
